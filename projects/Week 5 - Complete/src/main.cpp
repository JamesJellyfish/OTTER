...
int main() {
	...
	VertexArrayObject::sptr vao5 = ObjLoader::OBJLoader("monkey.obj");
	...
	while (!glfwWindowShouldClose(window)) {
		...
		transform4 = glm::translate(glm::mat4(1.0f), glm::vec3(1.8f, 0.0f, 0.0f)) * glm::rotate(glm::mat4(1.0f), static_cast<float>(thisFrame), glm::vec3(0, 1, 0));

		transform5 = glm::translate(glm::mat4(1.0f), glm::vec3(-2.0f, 0.0f, 0.0f)) * glm::rotate(glm::mat4(1.0f), static_cast<float>(thisFrame), glm::vec3(0, 0, 1));
	
		if (glfwGetKey(window, GLFW_KEY_SPACE) == GLFW_PRESS && camTyp == false)
		{
			camTyp = true;
			//camera->SetCam(true);
		}
		else if (glfwGetKey(window, GLFW_KEY_SPACE) == GLFW_RELEASE && camTyp == false)
		{
			camTyp = true;
